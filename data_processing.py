...
from util.data import split_data, binarize_classes
...
class HousingDataPreparation(DataPreparation):
    ...
    def data_prep(self, return_array=False):
        ...
        if self.target_pipe is not None:
            warnings.warn("Target pipeline is not needed for the Boston House Price dataset. "                           "Even though you passed a Pipeline for `target_pipe`, "                           "`target_pipe` will be set to None.")
...
class MNISTDataPreparation(DataPreparation):
    ...
    def data_prep(self, binarize=False, return_array=False):
        ...
        if return_array:
            if hasattr(y_trn_df, "toarray"):
                y_trn_df = y_trn_df.toarray()
            if hasattr(y_vld_df, "toarray"):
                y_vld_df = y_vld_df.toarray()
            return X_trn_df, y_trn_df, X_vld_df, y_vld_df
